...
function GoodEnoughSort({anime_list, set_sorted_list, set_is_sorted}: SorterProps) {
    const [sort_state, set_sort_state] = useState<GoodEnoughSortState<AnimeContent>>({
        ...
        list: anime_list,
        sort_buffer: [],
        buckets: Array(Math.ceil(Math.sqrt(anime_list.length))).fill([])
    })
    ...
    return (
        <div>
            ...
            <LinearProgress variant="determinate"
                            value={compares_count / (2 * Math.ceil(Math.sqrt(anime_list.length))) * 100}/>
        </div>
    )
}
